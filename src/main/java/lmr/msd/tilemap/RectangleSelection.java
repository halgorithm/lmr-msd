...
> {
    ...
    public void setRect(Rectangle rect) {
        if (!isInBounds(srcTileMap.data, rect))
            throw new IllegalArgumentException("Rectangle passed to RectangleSelection must be within lmr.msd.tilemap bounds");
        this.rect = rect;
    }
    ...
    public boolean contains(int x, int y) {
        return TileMapEdit.isInBounds(x, y, rect);
    }
    ...
    public S select(TileAndCoordsPredicate<T> condition) {
        var points = TileMapEdit.testTiles(
            srcTileMap.getData(), rect.x, rect.y, width(), height(), condition
        );
